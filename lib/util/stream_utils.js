...
shaka.util.StreamUtils = class {
  ...
  static chooseCodecsAndFilterManifest(manifest, preferredAudioChannelCount) {
    ...
    let variantsByCodecs = StreamUtils.getVariantsByCodecs(variants);
    variantsByCodecs = StreamUtils.filterVariantsByHeight(variantsByCodecs);

    const bestCodecs = StreamUtils.findBestCodecs(variantsByCodecs);
    ...
    manifest.variants = manifest.variants.filter((variant) => {
      const codecs = StreamUtils.getGroupVariantCodecs(variant);
   ...
   */
  static getVariantsByCodecs(variants) {
    const variantsByCodecs = new shaka.util.MultiMap();
    for (const variant of variants) {
      const group = shaka.util.StreamUtils.getGroupVariantCodecs(variant);
   ...
   *
   * Average bandwidth of H.265 codecs (vp9) is higher than H.264 codecs (avc1).
   * H.265 has 4K support and more variants.
   *
   * @param {!shaka.util.MultiMap.<shaka.extern.Variant>} variantsByCodecs
   * @return {!shaka.util.MultiMap.<shaka.extern.Variant>}
   */
  static filterVariantsByHeight(variantsByCodecs) {
    ...
    variantsByCodecs.forEach((codecs, variants) => {
      for (const variant of variants) {
        ...
        const key = variant.video.height;
        if (!heightByCodecs.has(key)) {
          heightByCodecs.set(key, new Set());
        }

        heightByCodecs.get(key).add(codecs);
      }
    });
    ...
    /** @type {!shaka.util.MultiMap.<shaka.extern.Variant>} */
    const filteredVariantsByCodecs = new shaka.util.MultiMap();
    variantsByCodecs.forEach((codecs, variants) => {
      for (const variant of variants) {
        if (
          variant.video &&
          heightByCodecs.has(variant.video.height) &&
          heightByCodecs.get(variant.video.height).size !== countCodecs
        ) {
          continue;
        }

        filteredVariantsByCodecs.push(codecs, variant);
      }
    });

    return filteredVariantsByCodecs;
  }
   ...
   * @return {string}
   */
  static findBestCodecs(variantsByCodecs) {
    let bestCodecs = '';
  ...
  /**
   * Get group of variant codecs.
   * @param {!shaka.extern.Variant} variant
   * @return {string}
   */
  static getGroupVariantCodecs(variant) {
