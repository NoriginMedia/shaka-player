...
shaka.dash.DashParser = class {
  ...
  async requestManifest_() {
    ...
    await this.parseManifest_(response.data, response.uri, response, request);
   ...
   * @private
   */
  async parseManifest_(data, finalManifestUri, response, request) {
    const Error = shaka.util.Error;
    const MpdUtils = shaka.dash.MpdUtils;

    const mpd = shaka.util.XmlUtils.parseXml(data, 'MPD');
    if (!mpd) {
      const rawXml = shaka.util.StringUtils.fromUTF8(data);
      throw new Error(
          Error.Severity.CRITICAL, Error.Category.MANIFEST,
          Error.Code.DASH_INVALID_XML, finalManifestUri, rawXml, 'MPD',
