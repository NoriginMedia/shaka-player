...
shaka.Player = function(mediaElement, dependencyInjector) {
  ...
  actions.set(this.drmNode_, (has, wants) => {
    shaka.log.debug('[NM DEBUG] Before onInitializeDrm_');

    const p = this.onInitializeDrm_(has, wants);
    return AbortableOperation.notAbortable(p);
  });
  actions.set(this.loadNode_, (has, wants) => {
    shaka.log.debug('[NM DEBUG] Before onLoad_');
  ...
  if (mediaElement) {
    shaka.log.debug('[NM DEBUG] Before attach in Player Init');
 ...
 * @export
 */
shaka.Player.version = 'v2.5.16-uncompiled';
...
shaka.Player.prototype.onInitializeDrm_ = async function(has, wants) {
  ...
  shaka.log.debug('[NM Debug] Player: Before initForPlayback ' +
    'in onInitializeDrm_');
  ...
  shaka.log.debug('[NM DEBUG] Player: After ' +
    'initForPlayback, before filterAllPeriods_');
...
shaka.Player.prototype.onLoad_ = async function(has, wants) {
  ...
  shaka.log.debug('[NM DEBUG] onLoad_ called');
  ...
  shaka.log.debug('[NM DEBUG] before attach');
  await this.drmEngine_.attach(mediaElement);
  shaka.log.debug('[NM DEBUG] after attach');
      ...
      for (const drmInfo of variant.drmInfos) {
        ...
        if (drmInfo.keySystem == curDrmInfo.keySystem) {
          for (const initData of (drmInfo.initData || [])) {
            shaka.log.debug('[NM DEBUG] Before newInitData in ' +
              'player:filterNewPeriod_');
