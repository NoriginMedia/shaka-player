...
shaka.media.DrmEngine = class {
  ...
  onKeyStatusesChange_(event) {
    ...
    shaka.log.v2('[NM] Key status changed for session', session);

    const found = this.activeSessions_.get(session);

    shaka.log.debug('[NM] Active session: ', found);

    const keyStatusMap = session.keyStatuses;

    shaka.log.debug('[NM] Key statuses: ', JSON.stringify(keyStatusMap));
    ...
    keyStatusMap.forEach((status, keyId) => {
      ...
      if (status == 'expired') {
        ...
        shaka.log.debug('[NM] Found expired key by STATUS: ', keyId);
      }
    ...
    shaka.log.debug('[NM] Session expiration: ', session.expiration);
    shaka.log.debug('[NM] Device time Date.now(): ', Date.now());
    shaka.log.debug('[NM] Ms until expiration: ', msUntilExpiration);
    shaka.log.debug('[NM] Has expired keys by STATUS: ', hasExpiredKeys);
