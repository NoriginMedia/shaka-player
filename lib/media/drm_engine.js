...
shaka.media.DrmEngine.prototype.attach = function(video) {
  if (!this.mediaKeys_) {
    shaka.log.debug('[NM DEBUG] attach: no media keys: ', this.mediaKeys_);
  ...
  shaka.log.debug('[NM Debug]drmengine:attach:setMediaKeys: ', setMediaKeys);
...
shaka.media.DrmEngine.prototype.queryMediaKeys_ = function(configsByKeySystem) {
  ...
  p = p.then(function(mediaKeySystemAccess) {
    ...
    shaka.log.debug(
      '[NM DEBUG] DRM Engine, mediaKeySystemAccess.createMediaKeys(): ',
      mediaKeys);
