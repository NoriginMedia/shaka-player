...
shaka.media.DrmEngine = class {
  ...
  onKeyStatusesChange_(event) {
    ...
    shaka.log.debug('Key status changed for session', session);

    const found = this.activeSessions_.get(session);

    shaka.log.debug('active session found', found);

    const keyStatusMap = session.keyStatuses;

    shaka.log.debug('Key statuses map', keyStatusMap);
    ...
    keyStatusMap.forEach((status, keyId) => {
      ...
      if (status == 'expired') {
        ...
        shaka.log.debug('Key status expired for keyId', keyId);
      }
    ...
    shaka.log.debug('session expiration', session.expiration);
    shaka.log.debug('ms until session expiration', msUntilExpiration);
    shaka.log.debug('has expired keys?', hasExpiredKeys);

    if (hasExpiredKeys && msUntilExpiration < 1000) {
